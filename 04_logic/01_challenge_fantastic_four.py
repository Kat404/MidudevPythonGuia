...
# ! Módulo 4 - Lógica - Ejercicio 1 (01_challenge_fantastic_four.py)
...
def check_is_balanced(text):
    ...
    # Contar fácilmente el número de veces que aparece una letra en una cadena.
    count_r = text.count("R") # Reed Richards
    count_j = text.count("J") # Johnny Storm

    print(f"count_r: {count_r}")
    print(f"count_j: {count_j}")

    return count_r == count_j

    # ? Esta porción de código es equivalente a la línea 38.
    # ? Es más largo y menos eficiente. Además, es menos legible.
    #if count_r == count_j:
    #    return True
    #else:
    #    return False
...
print(check_is_balanced("sdgkaghagfa"))
